...
const callGeminiAPI = async (prompt) => {
    ...
    const payload = { contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: { responseMimeType: "application/json", responseSchema: { type: "OBJECT", properties: { weeklyPlan: { type: "ARRAY", description: "A 5-day workout plan.", items: { type: "OBJECT", properties: { day: { type: "STRING" }, focus: { type: "STRING" }, exercises: { type: "ARRAY", items: { type: "OBJECT", properties: { name: { type: "STRING" }, sets: { type: "STRING" }, reps: { type: "STRING" } }, required: ["name", "sets", "reps"] } } }, required: ["day", "focus", "exercises"] } } } }, required: ["weeklyPlan"] } } };
...
const GeneratePlanView = ({ onCancel }) => {
    ...
    const handleGenerate = async (e) => { e.preventDefault(); if (!goal.trim()) return; setLoading(true); setError(null); setPlan(null); const prompt = `You are an expert personal trainer. Create a well-balanced, 5-day weekly workout plan for a user whose goal is to "${goal}". For each day, provide a focus (e.g., "Chest & Triceps") and a list of 4-5 exercises. For each exercise, specify the number of sets and reps. Provide the response as a valid JSON object adhering to the provided schema.`; try { const result = await callGeminiAPI(prompt); setPlan(result.weeklyPlan); } catch (err) { console.error("Gemini API error:", err); setError("Sorry, I couldn't generate a plan right now. Please try again later."); } finally { setLoading(false); } };
    return (<div className="bg-gray-800 p-6 rounded-lg shadow-lg space-y-6"><h2 className="text-2xl font-bold text-cyan-400 flex items-center"><BrainCircuit className="mr-3" /> AI Workout Plan Generator</h2><p className="text-gray-300">Tell us your fitness goal, and our AI will generate a personalized 5-day workout plan for you.</p><form onSubmit={handleGenerate} className="space-y-4"><div><label htmlFor="goal" className="block text-sm font-medium text-gray-300 mb-1">What is your primary fitness goal?</label><input id="goal" type="text" value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="e.g., build muscle, lose weight, improve endurance" className="w-full bg-gray-700 border-gray-600 rounded-lg p-3 focus:ring-cyan-500 focus:border-cyan-500" required /></div><div className="flex items-center justify-end space-x-4"><button type="button" onClick={onCancel} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg">Back to Dashboard</button><button type="submit" disabled={loading} className="bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg disabled:bg-gray-500 disabled:cursor-not-allowed flex items-center">{loading ? <Dumbbell className="w-5 h-5 animate-spin mr-2"/> : '✨ Generate Plan'}</button></div></form>{error && (<Modal title="Error" onClose={() => setError(null)}><p className="text-red-400">{error}</p></Modal>)}{plan && (<Modal title={`Your Plan for "${goal}"`} onClose={() => setPlan(null)}><WorkoutPlanDisplay plan={plan} /></Modal>)}</div>);
};
const Modal = ({ title, children, onClose }) => (<div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4"><div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-full overflow-y-auto"><div className="flex justify-between items-center p-4 border-b border-gray-700 sticky top-0 bg-gray-800"><h3 className="text-xl font-bold text-cyan-400">{title}</h3><button onClick={onClose} className="text-gray-400 hover:text-white"><X size={24} /></button></div><div className="p-6">{children}</div></div></div>);
const WorkoutPlanDisplay = ({ plan }) => (<div className="space-y-6">{plan.map((dayPlan, index) => (<div key={index} className="bg-gray-700/50 p-4 rounded-lg"><h4 className="text-lg font-bold text-cyan-400">{dayPlan.day}: <span className="text-yellow-400">{dayPlan.focus}</span></h4><ul className="mt-2 space-y-2">{dayPlan.exercises.map((exercise, i) => (<li key={i} className="flex justify-between items-center bg-gray-700 p-2 rounded"><span className="font-semibold text-gray-200">{exercise.name}</span><span className="text-gray-300">{exercise.sets} sets x {exercise.reps} reps</span></li>))}</ul></div>))}</div>);
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-800 p-6 rounded-lg shadow-md flex justify-between items-center">
                    <div><p className="text-gray-400 text-sm">Current Weight</p><p className="text-3xl font-bold">{currentWeight ? `${currentWeight} lbs` : 'N/A'}</p></div>
...
// --- Universal Workout Form Component (unchanged) ---
const WorkoutForm = ({ onSave, onCancel, existingWorkout = null }) => {
    const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
    const [currentWeight, setCurrentWeight] = useState('');
    const [cardio, setCardio] = useState([]);
    const [weights, setWeights] = useState([]);
    useEffect(() => { if (existingWorkout) { setDate(existingWorkout.date || new Date().toISOString().split('T')[0]); setCurrentWeight(existingWorkout.currentWeight || ''); setCardio(existingWorkout.cardio || []); setWeights(existingWorkout.weights || []); } }, [existingWorkout]);
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-lg shadow-lg space-y-6">
            ...
            <div><h3 className="text-lg font-semibold flex items-center text-yellow-400"><Zap size={18} className="mr-2"/>Cardio</h3>{cardio.map((c, i) => (<div key={i} className="grid grid-cols-1 md:grid-cols-4 gap-2 mt-2 p-3 bg-gray-700/50 rounded-lg"><input type="text" placeholder="Activity" value={c.type} onChange={e => handleCardioChange(i, 'type', e.target.value)} className="bg-gray-700 border-gray-600 rounded p-2" required /><input type="number" placeholder="Duration (min)" value={c.duration} onChange={e => handleCardioChange(i, 'duration', e.target.value)} className="bg-gray-700 border-gray-600 rounded p-2" required /><input type="number" placeholder="Distance (km)" value={c.distance} onChange={e => handleCardioChange(i, 'distance', e.target.value)} className="bg-gray-700 border-gray-600 rounded p-2" required /><button type="button" onClick={() => removeCardio(i)} className="text-red-500 hover:text-red-400 flex items-center justify-center p-2 rounded-lg bg-gray-600 hover:bg-gray-500"><Trash2 size={18}/></button></div>))}<button type="button" onClick={addCardio} className="mt-2 text-cyan-400 hover:text-cyan-300 flex items-center"><Plus size={16} className="mr-1"/>Add Cardio Activity</button></div>
            <div><h3 className="text-lg font-semibold flex items-center text-cyan-400"><Dumbbell size={18} className="mr-2"/>Weightlifting</h3><div className="space-y-4">{weights.map((ex, exIndex) => (<div key={exIndex} className="bg-gray-700/50 p-4 rounded-lg space-y-3"><div className="flex items-center justify-between"><input type="text" placeholder="Exercise Name (e.g. Bench Press)" value={ex.name} onChange={e => handleExerciseNameChange(exIndex, e.target.value)} className="flex-grow bg-gray-700 border-gray-600 rounded p-2 font-semibold" required /><button type="button" onClick={() => removeWeightExercise(exIndex)} className="ml-4 text-red-500 hover:text-red-400"><Trash2 size={18}/></button></div>{ex.sets.map((set, setIndex) => (<div key={setIndex} className="grid grid-cols-4 gap-2 items-center ml-4"><span className="text-gray-400 font-medium">Set {setIndex + 1}</span><input type="number" placeholder="Reps" value={set.reps} onChange={e => handleSetChange(exIndex, setIndex, 'reps', e.target.value)} className="bg-gray-600 border-gray-500 rounded p-2" required /><input type="number" placeholder="Weight (lbs)" value={set.weight} onChange={e => handleSetChange(exIndex, setIndex, 'weight', e.target.value)} className="bg-gray-600 border-gray-500 rounded p-2" required /><button type="button" onClick={() => removeSet(exIndex, setIndex)} className="text-red-500 hover:text-red-400"><X size={18}/></button></div>))}<button type="button" onClick={() => addSet(exIndex)} className="ml-4 mt-2 text-cyan-400 hover:text-cyan-300 flex items-center text-sm"><Plus size={14} className="mr-1"/>Add Set</button></div>))}<button type="button" onClick={addWeightExercise} className="mt-4 text-cyan-400 hover:text-cyan-300 flex items-center"><Plus size={16} className="mr-1"/>Add Weightlifting Exercise</button></div>
